...
function LandingPage() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:4000/sugang/get/` + localStorage.getItem('userId'))
      .then((response) => {
        if (response.data) {
            let obj = [];
            for (const sugang of response.data) {
                axios.get(`http://localhost:4000/subject/get/`+sugang.subject_id)
                .then(res => {
                    obj.push({
                        sugang: sugang,
  ...
  const renderCards_having = Sugang.map((sugang, index) => {
      if (sugang.sugang.timetable_number === TableNumber) {
              return (
                  <Col key={index} lg={6} md={8} xs={24}>
                      <span>과목명: {sugang.sugang.subject_name} </span>
                      <br />
    ...
    <div style={{ width: '85%', margin: '3rem auto' }}>
      ...
      <hr />
      <Row gutter={16}>{renderCards_having}</Row>
